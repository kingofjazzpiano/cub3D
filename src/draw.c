...
int	draw(t_all *all)
{
	...
	circumference(all, all->plr.pos, all->plr.radius, GREEN);
	player_update(&all->plr, all->key_flags, all->timers.delta_time);

	// Расчет времени отрисовки кадра : delta_time
	clock_gettime(CLOCK_REALTIME, &all->timers.stop);
	if (all->timers.stop.tv_nsec >= all->timers.start.tv_nsec)
		all->timers.delta_time = (double)(all->timers.stop.tv_nsec - all->timers.start.tv_nsec) / NANOSEC_PER_SEC;
	else if(all->timers.stop.tv_nsec < all->timers.start.tv_nsec)
		all->timers.delta_time = (double)(all->timers.stop.tv_nsec + NANOSEC_PER_SEC - all->timers.start.tv_nsec) / NANOSEC_PER_SEC;
	if (all->timers.stop.tv_sec > all->timers.start.tv_sec)
		all->timers.fps = (int)round(1.0 / all->timers.delta_time);
	clock_gettime(CLOCK_REALTIME, &all->timers.start);
	////////////////////////////////////////////////
	...
	// FPS
	char *fps_on_screen = ft_itoa(all->timers.fps);
	mlx_string_put(all->win.mlx, all->win.win2, all->win.width - 100, all->win.height - 100, GREEN, fps_on_screen);
	free(fps_on_screen);
	/////
	return (0);
}
